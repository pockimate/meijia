    ...
    <div class="w-full max-w-4xl bg-gray-50 border border-gray-200 shadow-lg overflow-hidden rounded-lg">
      ...
      <div class="bg-white border-b border-gray-200 p-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs tracking-[0.2em] uppercase font-light text-gray-500">ZODIAC NAIL STYLIST</p>
          </div>
        </div>
      </div>
      ...
      <div class="h-96 overflow-y-auto p-6 space-y-4">
        <div v-for="msg in messages" :key="msg.id" :class="['flex gap-4', msg.role === 'user' ? 'flex-row-reverse' : '']">
          <div :class="['w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-bold', msg.role === 'user' ? 'bg-gray-200 text-gray-900' : 'bg-black text-white']">
            {{ msg.role === 'user' ? 'U' : 'AI' }}
          </div>
      ...
      <div class="p-4 border-t border-gray-200 bg-white">
        ...
        <p class="text-xs tracking-[0.2em] uppercase font-light text-gray-500 mt-2 text-center">Note: This is a demo with preset responses.</p>
      </div>
    </div>
  </div>
</template>
