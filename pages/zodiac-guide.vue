    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="sign in signs" :key="sign.name" class="group relative bg-astro-card rounded-2xl border border-white/5 overflow-hidden hover:border-white/20 transition-all duration-300 hover:-translate-y-1">
          ...
          <div :class="`absolute inset-0 bg-gradient-to-br ${sign.color} opacity-0 group-hover:opacity-5 transition-opacity duration-500 pointer-events-none`"></div>
        </div>
      </div>
    </div>
  </div>
...
useHead({
  title: 'Zodiac Guide | Astro Nails'
});
</script>
